...
def create_dataset(repeat_num=1, training=True, batch_size=64):
    ...
    if usage == 'train':
        data_bd_train = np.load(f'./poison_data/cifar10/blended10/train_bd.npz')
        iter_bd_train = poison_iterator(data = data_bd_train['data'], label = data_bd_train['labels'])
        cifar_ds = GeneratorDataset(source=iter_bd_train, column_names=["image", "label"])
    else:
        data_clean_test = np.load(f'./poison_data/cifar10/blended10/test_clean.npz')
        iter_clean_test = poison_iterator(data = data_clean_test['data'], label = data_clean_test['labels'])
        cifar_ds = GeneratorDataset(source=iter_clean_test, column_names=["image", "label"])
